...
在开始新故事之前，先修掉了 实验脚本 潜伏已久的几个问题。

#### Bug 1：`set -e` 让整个循环只跑一次就退出

原因：bash 的 `set -e` 会在**任何**非零返回码时退出，包括 `if ... then ... fi` 分支内部的命令。实验脚本 里有两处 `cd "$REPO_ROOT"`：一处在 claude 调用前，一处在 guardrail 检查前。这两处都没有错误处理，一旦 cd 失败（路径不存在、权限问题等），bash 直接 `exit`，而不是 `continue` 跳过这次迭代。
...
原因：`guardrail.py` 默认 `tol=0.0` 意味着任何一轮的 RMSE 稍微比上一轮高就判定"回退"。但 Optuna 的 TPE 采样器在相同种子下**并非完全确定性**——两次相同参数的运行可能差出 ~0.001 RMSE。于是每次 回退到上一个 commit，重跑，RMSE 又略有不同，再次失败，进入死循环直到 max_iterations。
...
原因：Claude Code 运行时会往环境里写 `CLAUDECODE` 变量。子进程再启动 `claude` 时，检测到这个变量，拒绝启动（"nested sessions share runtime resources"）。如果用户在 Claude Code 的终端里执行 `实验脚本 --tool claude`，第一个 story 的 agent 实际上根本没跑。
...
同步修改了 `optimize_per_target()`，加入 `inverse_transforms` 和 `y_original` 参数：Optuna 目标函数在调参时用 cross_val_predict 得到 logit 预测，经逆变换后再算 RMSE，保证选到的 alpha 是在 wt% 空间最优的（不是 logit 空间最优的）。
...
原有 22 个 ML-001~ML-022 故事全部替换为 23 个 OES-001~OES-023 故事，主分支改为 `main`。
...
2026-02-25  完成 OES-001~020（25 次迭代实验）
